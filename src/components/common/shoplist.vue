          ...
          <h5 class="rating_order_num">
            <section class="rating_order_num_left">
              <section class="rating_section">
                <van-rate v-model="item.rating" void-icon="star" :size="10" color="#ffd21e" void-color="#eee" />
  ...
  methods: {
    async initData() {
      ...
      this.shopListArr = res.map((item,index)=>{
        ...
        item.name = '商家名称'+(index+1)
        item.image_path = require('@/assets/images/shop_img.png')
        return item
      });
